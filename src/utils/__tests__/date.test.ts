...
describe('getFormattedDate', () => {
  test('formats a Date object with the site locale', () => {
  ...
  test('accepts a date string and formats it', () => {
  ...
  test('returns "Invalid Date" when input is undefined', () => {
  ...
  test('returns "Invalid Date" for an invalid Date object', () => {
    expect(getFormattedDate(new Date('invalid'))).toBe('Invalid Date')
  })

  test('respects custom month option passed as override', () => {
    const date = new Date('2024-01-15T12:00:00Z')
    const formatted = getFormattedDate(date, { month: 'long' })
    expect(formatted).toContain('January')
  })

  test('returns a consistent result across multiple calls with the same input', () => {
...
describe('collectionDateSortProjects', () => {
  ...
  test('returns a negative value when the first project is newer', () => {
    const older = makeProject(new Date('2021-06-01'))
    const newer = makeProject(new Date('2024-06-01'))
    expect(collectionDateSortProjects(newer, older)).toBeLessThan(0)
  })

  test('returns 0 for projects with the same start date', () => {
  ...
  test('sorts an array newest-first when used with Array.sort', () => {
    const projects = [
